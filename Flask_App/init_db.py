...
def main():
    ...
    cur.execute(
        "SELECT setval('accountcontrol_id_seq', (SELECT MAX(id) FROM accountcontrol)+1);"
    )
    ...
    cur.execute(
        'SELECT setval(\'"Interest_id_seq"\', (SELECT MAX(id) FROM "Interest")+1);'
    )
    ...
    cur.execute(
        'SELECT setval(\'"ProfilInterest_id_seq"\', (SELECT MAX(id) FROM "ProfilInterest")+1);'
    )
