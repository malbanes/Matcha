...
def matching_calculation(
    orientation, long, lat, city, interest_num, birthdate, popularity_score
):
    ...
    cur.execute(
        "SELECT * FROM profil WHERE user_id != {0} {1}".format(
            current_user.id, select_gender
        )
    )
    users = cur.fetchall()
    user_score = 0
    ...
    for user in users:
        if orientation == user[3]:
            user_score += 60
        cur.execute(
            "SELECT latitude, longitude, city FROM location WHERE id=%(location_id)s LIMIT 1",
            {"location_id": user[4]},
        )
        ...
        cur.execute(
            'Select COUNT(pi.id) FROM profil LEFT JOIN "ProfilInterest" as pi ON profil.user_id = pi.user_id AND pi.user_id = %(targeted_user)s AND pi.interest_id IN (SELECT interest_id FROM "ProfilInterest" WHERE user_id=%(current_user_id)s ) GROUP BY profil.user_id ORDER BY COUNT(pi.id) desc LIMIT 1',
            {"targeted_user": user[1], "current_user_id": current_user.id},
        )
        ...
        if age(user[5]) != "N/A" and birthdate != "N/A":
            ...
            if birthdate == age(user[5]):
                ...
                birthdate >= minus_targeted_user_age
                and birthdate <= max_targeted_user_age
            ):
                user_score += 20

        if popularity_score <= (user[8] + 500) and popularity_score >= (user[8] - 500):
            user_score += 20
        elif popularity_score <= (user[8] + 500) and popularity_score >= (
            user[8] - 500
        ):
            user_score += 15
        elif popularity_score <= (user[8] + 1000) and popularity_score >= (
            user[8] - 1000
        ):
            user_score += 10
        ...
        if user_score > 25:
            cur.execute(
                "INSERT INTO match (user_id, match_id, score) VALUES (%(user_id)s, %(match_id)s, %(score)s)",
                {
