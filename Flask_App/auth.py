...
def login():
    ...
    # if the request is POST, then check if the user exist and with the right password
        ...
        cur.execute(
            "UPDATE profil SET is_online = true WHERE user_id=%(id)s",
            {"id": current_user.id},
        )
        cur.execute(
            "SELECT score from profil WHERE user_id=%(id)s LIMIT 1",
            {"id": current_user.id},
        )
        ...
        cur.execute(
            "SELECT COUNT(*) from likes WHERE receiver_id=%(id)s",
            {"id": current_user.id},
        )
        likes_num = int(cur.fetchone()[0])
        cur.execute(
            'SELECT COUNT(*) from "ProfilInterest" WHERE user_id=%(id)s',
            {"id": current_user.id},
        )
        tag_num = int(cur.fetchone()[0])
        cur.execute(
            "SELECT COUNT(*) from accountcontrol WHERE to_user_id=%(id)s and blocked=true",
            {"id": current_user.id},
        )
        block_num = int(cur.fetchone()[0])
        cur.execute(
            "SELECT COUNT(*) from accountcontrol WHERE to_user_id=%(id)s and fake=true",
            {"id": current_user.id},
        )
        report_num = int(cur.fetchone()[0])
        cur.execute(
            "SELECT last_log from profil WHERE user_id=%(id)s", {"id": current_user.id}
        )
        lact_co_date = cur.fetchone()[0]
        cur.execute(
            "SELECT COUNT(DISTINCT receiver_id) from messages WHERE sender_id=%(id)s",
            {"id": current_user.id},
        )
        match_num = int(cur.fetchone()[0])
        updated_score = scoring_calculation(
            former_score,
            image_num,
            likes_num,
            tag_num,
        ...
        cur.execute(
            "UPDATE profil SET score = %(score)s WHERE user_id=%(id)s",
            {"score": updated_score, "id": current_user.id},
        )
        conn.commit()
        cur.close()
        conn.close()
...
def signup():
    ...
    # if request is POST, then check if the email doesn't already exist and then save data
        ...
        if (
            len(str(email)) > 250
            or len(str(first_name)) > 64
            or len(str(last_name)) > 64
            or len(str(username)) > 64
            or len(str(email)) < 1
            or len(str(first_name)) < 1
            or len(str(last_name)) < 1
            or len(str(username)) < 1
            or match is None
        ):
            flash(
                "There was an issue with your inputs, try again",
            )
            return redirect(url_for("auth.signup"))
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT email FROM users WHERE email=%(email)s OR username = %(username)s",
        ...
        if pass_complexity["password_ok"] == False:
            ...
            cur.execute(
                "INSERT INTO users (email, first_name, last_name, username, password, confirmed) VALUES (%(email)s, %(first_name)s, %(last_name)s, %(username)s, crypt(%(password)s, gen_salt('bf')), false);",
                {
                    "email": email,
            ...
            loc_id = cur.execute(
                "INSERT INTO location (latitude,longitude,date_modif,city) VALUES (%(lat)s, %(long)s, %(date)s, %(city)s) returning id",
                {
                    "lat": latitude,
                    "long": longitude,
            ...
            loc_id = cur.fetchone()
            cur.execute(
                "INSERT INTO profil (user_id, location_id, score) VALUES (%(user_id)s, %(location_id)s, 0)",
                {"user_id": new_user[0], "location_id": loc_id[0]},
            )
            conn.commit()
            cur.execute(
                "UPDATE profil SET is_online = true WHERE user_id=%(id)s",
...
def confirm_email(token):
    ...
    if user[7] == True:
        ...
        cur.execute(
            "UPDATE users SET confirmed = true WHERE email=%(email)s", {"email": email}
        )
        conn.commit()
...
def reset_page():
    if request.method == "GET":
        ...
        if not (re.fullmatch(email_regex, email)) or len(str(email)) > 250:
            ...
            if user:
                ...
                html = render_template(
                    "reset_password_request.html", reset_url=reset_url
                )
    ...
    if request.method == "POST":
        ...
        if pass_complexity["password_ok"] == False:
            ...
            if user and user[7] == True:
                cur.execute(
                    "UPDATE users SET password = crypt(%(password)s, gen_salt('bf')) WHERE email=%(email)s",
...
def logout():
    ...
    cur = conn.cursor()
    cur.execute(
        "UPDATE profil SET is_online = false, last_log = %(date)s WHERE user_id=%(id)s",
        {"date": str(datetime.date(datetime.now())), "id": current_user.id},
    )
    conn.commit()
    cur.close()
    conn.close()
